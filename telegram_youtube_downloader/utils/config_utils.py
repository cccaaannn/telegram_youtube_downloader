...
from telegram_youtube_downloader.data.app_config import AppConfig
...
class ConfigUtils:
	...
	@staticmethod
	def __set_value_from_nested_keys_recursive(
		cfg: dict, keys: List[str], current_key_index: int, value: str
	):
		"""
		...
		# Recursive call
		ConfigUtils.__set_value_from_nested_keys_recursive(
			cfg[current_key], keys, current_key_index + 1, value
		)
	...
	def __deserialize_config(cfg_dict: dict) -> AppConfig:
		return dacite.from_dict(
			...
			data=cfg_dict,
			config=DaciteConfig(type_hooks={int: int}, cast=[Enum]),
		)
	...
	def init_config():
		...
		for key, value in os.environ.items():
			keys = (
				[key]
				if ConfigUtils.__nested_env_key_separator not in key
				else key.split(ConfigUtils.__nested_env_key_separator)
			)
	...
	def get_app_config() -> AppConfig:
		if not ConfigUtils.__app_config:
			raise Exception(
				f"Config not initialized. Call {ConfigUtils.init_config.__name__} first"
			)
