...
from telegram_youtube_downloader.telegram_bot import TelegramBot
...
class Cli:
	...
	def start(self):
		parser = argparse.ArgumentParser(
			description="Youtube telegram downloader",
			epilog=f"Uses environment variable <{self.bot_key_env_name}> for the Telegram bot key if no arguments passed",
		)
		...
		mutually_exclusive_group.add_argument(
			...
			metavar="<key>",
			type=str,
			help="Telegram bot and YouTube api key",
		)
		mutually_exclusive_group.add_argument(
			"-f",
			"--use_file",
			dest="use_file",
			metavar="<path>",
			type=Cli.__path_type,
			help="File path for Telegram bot and YouTube api key",
		)
		...
		if not os.environ.get(self.bot_key_env_name, None):
			self.__logger.error(
				f"A telegram bot key must presents at {self.bot_key_env_name} environment variable"
			)
			sys.exit(1)
