class TelegramBotErrorHandler:
    ...
    def menu_handler(logger):
        ...
        def decorator(func):
            def wrapper(*args, **kwargs):
                try:
                    logger.info(f"Function: {func.__name__} User: {args[0].callback_query.message.from_user}")
                    func(*args, **kwargs)
                except:
                    logger.error(f"function: {func.__name__} User:{args[0].callback_query.message.from_user}", exc_info=True)
